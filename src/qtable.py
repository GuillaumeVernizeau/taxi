...
def qtable(args: QParameters):
    ...
    if args.optimize_with_variations or args.optimize_with_bayesian:  # Vérifier si l'optimisation est activée
        print(f"\n{BG_CYAN}----- OPTIMISATION DES HYPERPARAMÈTRES -----{OFF}")
        optimizer = Optimizer(base_params=args)  # Créer une instance de Optimizer
        args = optimizer.optimize()  # Appeler la méthode optimize() pour obtenir les meilleurs paramètres

...
def run_deep_q(args: QParameters):
    ...
    if not args.skipTraining:
        ...
        # train_stats = agent.train(args.training_episodes)
        train_stats = agent.train(args.training_episodes, [args.map_path])
        agent.save()  # ← à ajouter pour sauvegarder le modèle
...
def train_on_multiple_maps(map_paths: List[str], episodes_per_map: int, args: QParameters):
    ...
    # 🎮 Phase de test sur une carte de test (potentiellement plus grande)
    test_map = "tester/5x5_test.map"  # ou "maps/12x12.map" si tu veux tester la généralisation
    print(f"\n🧪 Test du modèle sur : {test_map}")
    agent.env = CustomTaxiEnv(mapPath=test_map)
    agent.play(episodes=args.play_episodes, render=False)  # ou False si tu veux désactiver l'affichage
